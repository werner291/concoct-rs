...
proptest! {
    ...
    fn calc_sample_var_matches_c(
        ...
        n_dims in 1usize..32,
        seed in prop::collection::vec(common::adversarial_f64(), 128 * 32),
    ) {
        ...
        for i in 0..n_dims {
            assert_eq!(
                rust_mu[i].to_bits(), c_mu[i].to_bits(),
                "mu mismatch at dim={i}: rust={:e} vs c={:e}",
                rust_mu[i], c_mu[i],
            );
            assert_eq!(
                rust_var[i].to_bits(), c_var[i].to_bits(),
                "var mismatch at dim={i}: rust={:e} vs c={:e}",
                rust_var[i], c_var[i],
            );
        }
    }
